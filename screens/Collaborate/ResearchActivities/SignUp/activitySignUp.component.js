...
export function ActivitySignUpPage(props) {
  ...
  const onBeginPress = async (timeSlot, index) => {
    ...
    // access test
      ...
      props.navigation.navigate("AccessTest")
    }
  ...
  const signUpTimeSlot = async (timeSlot, route) => {
    ...
    let res = null
    try {
        const response = await fetch(HEROKU_SERVER+'/' + route + timeSlot._id + '/claim', {
            method: 'PUT',
            headers: {
          ...
          :
            ...
            <Text>
              ...
              props.activity.test_type === activityList[9] ?
                ...
                "Time at Site:"
              : 
                //If test does have standing points
                "Time per Standing Point:"
              )} {props.activity.duration} (min)
            </Text>
          }
