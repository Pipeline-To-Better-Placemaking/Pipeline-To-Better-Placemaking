...
export function LightResultPage(props) {
  ...
  const refreshDetails = async () => {
    if (props.selectedResult !== null && props.selectedResult.sharedData !== undefined) {
      ...
      result = await formatLightGraphData(result);
  ...
  const deleteResult = async () => {
    ...
    if (props.selectedResult !== null) {
      success = await deleteTimeSlot("light_maps", props.selectedResult._id);
    }
    if (success) {
      await refreshProjectPageDetails();
  ...
  const viewMapResults = () => {
    props.navigation.navigate("LightMapResultsView");
  }
    ...
    <ViewableArea>
      ...
      <ContentContainer>
        ...
        >
          ...
          <Text category={'h5'}>Lighting Profile Results</Text>
