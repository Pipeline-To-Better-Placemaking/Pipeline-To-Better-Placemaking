...
export function CreateActivityStack(props) {
  ...
  const postCollection = async (name, test_type, collectionName, timeSlotName) => {
    ...
    // Save the activity
    try {
        const response = await fetch(HEROKU_SERVER+'/projects/' + props.project._id + collectionName, {
            method: 'POST',
            headers: {
  ...
  const postTimeSlot = async (timeSlot, name, id, timeSlotName, test_type) => {
    ...
    // Save the activity
    try {
        const response = await fetch(HEROKU_SERVER+'/' + timeSlotName, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                    'Content-Type': 'application/json',
  ...
  const putCollection = async (name, test_type, collectionName, timeSlotName) => {
    ...
    // Save the activity
    try {
        const response = await fetch(HEROKU_SERVER+'/projects/' +
                                      props.project._id +
                                      collectionName  +
                                      '/' + props.activity._id, {
            method: 'PUT',
  ...
  const putTimeSlot = async (timeSlot, name, id, timeSlotName, test_type) => {
    ...
    // Save the activity
    try {
        const response = await fetch(HEROKU_SERVER+'/' + timeSlotName + timeSlot._id, {
          method: 'PUT',
          headers: {
            Accept: 'application/json',
              'Content-Type': 'application/json',
