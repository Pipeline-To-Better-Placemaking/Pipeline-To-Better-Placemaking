...
export default function MapDrawer(props) {
    ...
    const charts = (selections, stationary, moving, order, boundaries, lighting, nature, sound, section) => (
        <>
            ...
            { Object.entries(drawers).map(([name, data]) => (
                <React.Fragment key={ menuAnchors[name] }>
                    ...
                    >
                        ...
                        { menuAnchors[name] === 'left' ? 
                            list(name, data) 
                            : (menuAnchors[name] === 'bottom' ? dataDrawer(selections) : (menuAnchors[name] === 'right' ? charts(selections, stationary, moving, order, boundaries, lighting, nature, sound, section) : null)) }
                    </Drawer>
                </React.Fragment>
            ))}
        </div>
